...
print(f"Сумма ряда (вычисление через цикл): {cycle_sum(n)}" )
print(f"Сумма ряда (рекурсивное вычисление): {recursion_sum(n, 1)}")

print("время вычисления через цикл:", timeit.timeit(("cycle_sum(n)"), setup="from __main__ import cycle_sum, n"))
print("время вычисления через рекурсию:",
      timeit.timeit(("recursion_sum(n, 1)"), setup="from __main__ import recursion_sum, n"))
"""
timeit для цикла при n = 600 и 1 млн. повторений показывает время - 30.20 с
timeit для рекурсии при n = 600 и 1 млн. повторений показывает время - 80.49 с
Код через циклы эффективнее по скорости вычислений как минимум в 2 раза + затрагивается меньше памяти
Однако это заметно лишь при больших нагрузках.
"""
